/**
 * ACC-6 · La tabla membership_scope (0034 §6 y §10: su RLS y los
 * privilegios de mc_app) y la migración 0040_scope_allows: la semántica
 * de scope_allows() y que se puede volver a aplicar. Las tres
...
describe('la tabla y su aislamiento', { timeout: 600_000 }, () => {
  ...
  test('el alcance se va con la membresía (ON DELETE CASCADE)', async () => {
    await t.admin(`
      INSERT INTO membership (workspace_id, user_id, role_id) VALUES ('${WORKSPACE_AJENO}', '${USER_MIEMBRO}', system_role_id('creator', 'manager')) ON CONFLICT DO NOTHING;
      INSERT INTO membership_scope (workspace_id, user_id, scope_type, scope_id)
  ...
  test('la migración se puede volver a aplicar sin fallar y deja lo mismo', async () => {
    const sql = await readFile(fileURLToPath(new URL('../../../db/migrations/0040_scope_allows.sql', import.meta.url)), 'utf8');
    await t.admin(sql);
