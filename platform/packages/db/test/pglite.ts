 ...
 *     db/seed aplicados, migrado como un rol sin BYPASSRLS y con la
 *     sesión como mc_app. Nada que instalar, nada que levantar, sin red.
 *     Las transacciones se serializan.
...
export async function openTestDb(opts: TestDbOptions = {}): Promise<TestDb> {
  ...
  const db = await createEmbeddedDb({ seeds: opts.seeds ?? true });
  return {
    kind: 'pglite',
    db,
