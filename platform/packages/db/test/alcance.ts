 ...
 *   Miembro una persona nueva con membresía de Mánager (el rol de
 *           fábrica pensado para «solo lo de sus creadores», 0034 §4) y
 *           UNA fila de alcance: creator = Laura. No puede ver ni tocar
 *           nada de Sofía.
...
export async function sembrarAlcance(t: TestDb): Promise<void> {
  const miembro = (userId: string, email: string, scopeType: string, scopeId: string) => `
    ...
    INSERT INTO membership (workspace_id, user_id, role_id) VALUES ('${WORKSPACE_LAURA}', '${userId}', system_role_id('creator', 'manager')) ON CONFLICT DO NOTHING;
 ...
 * de las tablas que escriben los tres módulos, y cuántas son. Igual antes
 * y después de la pasada del miembro = no escribió ni una fila.
 *
 * Se lee COMO LA DUEÑA, dentro del workspace de Laura. La primera versión
 * leía con t.raw(), que corre como mc_app sin workspace fijado: RLS
 * escondía todas las filas y la huella comparaba vacío con vacío (lo
 * encontró el cierre de ACC, 23-sep). El conteo va junto al md5 para que
 * una tabla vacía se vea como tal en el mensaje, y huellaDeVerdad() falla
 * si las tablas que siembra el escenario salen vacías.
 */
export async function huella(t: TestDb): Promise<Record<string, string | null>> {
  const tablas = [
    'campaign', 'campaign_post', 'invoice', 'payment', 'tax_reserve', 'social_connection', 'data_consent', 'account_metric_snapshot',
    'connection_secret', 'notification', 'audit_log',
  ];
  return t.db.withWorkspace(WORKSPACE_LAURA, async (tx) => {
    const out: Record<string, string | null> = {};
    for (const tabla of tablas) {
      const { rows } = await tx.query<{ h: string | null }>(
        `SELECT count(*)::text || ':' || md5(coalesce(string_agg(x::text, '|' ORDER BY x::text), '')) AS h FROM ${tabla} x`,
      );
...
/** Las tablas que el escenario siembra no pueden salir vacías en la huella: si salen, la huella no ve nada. */
export function huellaDeVerdad(h: Record<string, string | null>): string[] {
  return ['campaign', 'invoice', 'payment', 'social_connection', 'data_consent'].filter((tabla) => (h[tabla] ?? '0:').startsWith('0:'));
}
 ...
 * `sembrarExtra` añade al escenario las filas de Sofía que solo usa ese
 * módulo (gastos, pagos de plataforma, reportes…), después del común y
 * antes de la huella.
 */
export function definirPruebasDeAlcance(
  ...
  sembrarExtra?: (t: TestDb) => Promise<void>,
): void {
  let t: TestDb;
  ...
  describe(`alcance en queries/${nombre}.ts`, { timeout: 900_000 }, () => {
    ...
    test('la pasada del miembro no escribió ni una fila (huella leída como la dueña, no vacía)', async () => {
      assert.deepEqual(huellaDeVerdad(antes), [], 'la huella no ve las filas del escenario: se está leyendo sin workspace');
