...
test('1 · arranca como mc_worker, lee las definiciones y el log dice cuáles tienen handler', async () => {
  ...
  assert.equal(listo?.['withHandler'], 10);      // oauth.refresh + collect.account_metrics + collect.demographics (CON-7) + brand.snapshot (CAM-3) + campaign.compute (CAM-5) + 5 de prueba (test.off está apagado)
  assert.equal(listo?.['withoutHandler'], 16);
  ...
  assert.equal(skipped.rows.length, 16);
  assert.ok(skipped.rows.every((r) => r.error === 'sin handler' && Number(r.n) === 1));
  assert.ok(!skipped.rows.some((r) => ['oauth.refresh', 'collect.account_metrics', 'collect.demographics', 'brand.snapshot', 'campaign.compute', 'test.off'].includes(r.job_id)));
...
test('idempotencia de cron: reiniciar no duplica schedules y un cron cambiado se actualiza', async () => {
  ...
  try {
    ...
    // (ahora no tienen handler). Los 16 de 0009 sin handler no se repiten.
    ...
    assert.equal(Number(skipped.rows[0]!.n), 16, 'el reinicio no vuelve a insertar filas skipped');
