...
test('1 · arranca como mc_worker, lee las definiciones y el log dice cuáles tienen handler', async () => {
  ...
  assert.equal(listo?.['withHandler'], 8);       // oauth.refresh + collect.account_metrics + brand.snapshot + 5 de prueba (test.off está apagado)
  ...
  assert.ok(!skipped.rows.some((r) => r.job_id === 'oauth.refresh' || r.job_id === 'collect.account_metrics' || r.job_id === 'brand.snapshot' || r.job_id === 'test.off'));
