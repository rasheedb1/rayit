...
test('localhost, Docker y un Postgres cualquiera van sin opción ssl (manda la URL)', () => {
  assert.equal(tlsFor('postgres://mc:mc@localhost:5432/oncue', null), false);
  assert.equal(tlsFor('postgres://mc:mc@localhost/oncue', null), false, 'sin puerto también');
  assert.equal(tlsFor('postgres://mc:mc@db:5432/oncue', null), false);
