...
describe("ReporteSeccion (CAM-6)", () => {
  it("una campaña planeada sin reporte explica por qué no hay nada que generar", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="planned" reports={[]} origin="https://on-cue.test" canGenerate canSend f={f} />);
  ...
  it("un rol sin campanas.reporte.generar ni .enviar ve el estado y el enlace, no los botones, y la frase lo dice", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="measuring" reports={[reporte()]} origin="https://on-cue.test" canGenerate={false} canSend={false} f={f} />);
  ...
  it("sin reporte ofrece «Generar reporte» con lo que hace", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="measuring" reports={[]} origin="https://on-cue.test" canGenerate canSend f={f} />);
  ...
  it("un borrador: enlace absoluto con el origen público, aviso de que no abre y los dos «Enviado»", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="measuring" reports={[reporte()]} origin="https://on-cue.test" canGenerate canSend f={f} />);
  ...
  it("enviado y abierto: fechas en frases, sin botones de enviar, y «Generar de nuevo» explica las versiones", () => {
    ...
    render(<ReporteSeccion campaignId={CAMPANA} status="reported" reports={[r]} origin="https://on-cue.test" canGenerate canSend f={f} />);
  ...
  it("sin origen público no inventa un enlace: muestra la ruta y por qué", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="measuring" reports={[reporte()]} origin={null} canGenerate canSend f={f} />);
  ...
  it("un borrador de una campaña cancelada después no ofrece enviarlo", () => {
    render(<ReporteSeccion campaignId={CAMPANA} status="cancelled" reports={[reporte()]} origin="https://on-cue.test" canGenerate canSend f={f} />);
    expect(screen.queryByRole("button", { name: t.marcarEnlace })).toBeNull();
    expect(screen.queryByRole("button", { name: t.regenerar })).toBeNull();
  });
  ...
  it("con varias versiones las lista y marca la reemplazada", () => {
    ...
    render(<ReporteSeccion campaignId={CAMPANA} status="reported" reports={[nueva, vieja]} origin="https://on-cue.test" canGenerate canSend f={f} />);
