 ...
 * BY de la consulta y la proyección, de `proyectarGastos` de core, la
 * misma que usa el flujo de caja.
 */
...
export function barrasProyeccion(p: ProyeccionGastos, f: Formatter): BarrasProyeccion {
  return {
    cats: p.semanas.map((s) => f.dayMonthRange(s.inicio, s.fin)),
    axisLabels: p.semanas.map((s) => f.dayMonth(s.inicio)),
    data: p.semanas.map((s) => parseDecimal(s.gastos)),
  };
}
