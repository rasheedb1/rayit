...
import { proyectarGastosRecurrentes } from "@mc/core";
...
describe("barrasProyeccion", () => {
  const gastos = [
    { id: "a", category: "edicion", vendor: "Mateo", description: null, amount: "1800000.00", currency: "COP", incurredOn: "2026-09-01", recurrence: "monthly" },
  ];

  it("ocho barras, la etiqueta corta bajo la barra y el rango entero en el tooltip", () => {
    const p = proyectarGastosRecurrentes(gastos, "2026-09-23");
    const b = barrasProyeccion(p, f);
    ...
    // La barra lleva el número de la semana, que sale del string de core.
    expect(b.data[1]).toBe(1800000);
    expect(b.data[0]).toBe(0);
  });

  it("sin nada que proyectar, ocho barras en cero: la tarjeta lo explica con su estado vacío", () => {
    const b = barrasProyeccion(proyectarGastosRecurrentes([], "2026-09-23"), f);
