 ...
 */
const consultas = vi.hoisted(() => ({ getExpenseMonth: vi.fn(), listRecurringExpenses: vi.fn() }));
...
vi.mock("@/components/ui/chart-card", () => ({ ChartCard: () => <div data-testid="grafico" /> }));
...
async function pintar(m: Record<string, unknown>, recurrentes: Record<string, unknown> = {}) {
  consultas.getExpenseMonth.mockResolvedValue(mes(m));
  consultas.listRecurringExpenses.mockResolvedValue({ rows: [], today: "2026-09-23", currency: "COP", otherCurrencyCount: 0, ...recurrentes });
  render(await GastosPage({ searchParams: Promise.resolve({}) }));
}

beforeEach(() => {
  consultas.getExpenseMonth.mockReset();
  consultas.listRecurringExpenses.mockReset();
});
...
describe("la proyección de gastos recurrentes", () => {
  ...
  it("los recurrentes en otra moneda se cuentan con su frase, con gráfico y sin él", async () => {
    await pintar({}, { rows: [], otherCurrencyCount: 2 });
