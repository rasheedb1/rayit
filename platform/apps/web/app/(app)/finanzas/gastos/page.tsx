...
export default async function GastosPage({ searchParams }: { searchParams: Promise<{ mes?: string }> }) {
  ...
  // nada. Sin finanzas.gasto.ver, 404 como el resto del módulo, y la
  // pestaña no se pinta.
  ...
  // La proyección la calcula core con LA regla del flujo de caja
  // (proyectarGastos); `today` es el día del workspace, no el reloj del
  // proceso que sirve la petición.
  const proyeccion = proyectarGastos(entradas);
  const barras = barrasProyeccion(proyeccion, f);
  const otraMonedaRecurrente = proyeccion.otraMoneda.count;
  // Sin ningún recurrente NO se pinta el gráfico: ocho barras en cero no
  // explican nada, y el `emptyState` de ChartCard no se dispara porque
  // las series sí traen ocho valores (que son ceros de verdad).
  const hayProyeccion = !proyeccion.vacio;
    ...
    <>
      ...
      <section className="mt-10" aria-labelledby="gastos-del-mes">
        ...
        <GastosPanel
          ...
          puedeRegistrar={can(permisos, "finanzas.gasto.registrar")}
        />
      </section>
      ...
      <section className="mt-10" aria-labelledby={hayProyeccion ? undefined : "proyeccion"}>
        {!hayProyeccion ? (
          <>
            ...
            {otraMonedaRecurrente > 0 && (
              <p className="mt-2 text-xs text-muted">{T.proyeccion.otraMoneda(otraMonedaRecurrente)}</p>
            )}
          </>
        ) : (
        <ChartCard
          ...
          format="money-full"
          axisFormat="compact"
          currency={entradas.currency}
          labelsHeader={T.proyeccion.semana}
          legend={false}
          note={
            <>
              {T.proyeccion.nota(
                f.date(proyeccion.semanas[0]!.inicio),
                f.date(proyeccion.semanas[proyeccion.semanas.length - 1]!.fin),
                f.money(proyeccion.total, entradas.currency, { mode: "full" }),
                f.month(proyeccion.mes ?? mes.month),
                f.money(proyeccion.mensual, entradas.currency, { mode: "full" }),
              )}
              {proyeccion.nuevasDelMes > 0 && <span className="mt-1 block">{T.proyeccion.nuevas(proyeccion.nuevasDelMes)}</span>}
              {otraMonedaRecurrente > 0 && (
                <span className="mt-1 block">{T.proyeccion.otraMoneda(otraMonedaRecurrente)}</span>
              )}
            </>
          }
        />
