...
describe("guardarGasto · errores de dominio", () => {
  ...
  it("un fallo inesperado no se queda callado", async () => {
    createExpense.mockRejectedValue(new Error("la base no respondió"));
    const state = await guardarGasto({}, form());
    expect(state.message).toBe("la base no respondió");
  });
});
