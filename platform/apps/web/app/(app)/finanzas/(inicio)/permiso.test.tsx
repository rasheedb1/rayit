...
describe("leer las facturas pide finanzas.factura.ver (ACC-1)", () => {
  it("el rol Editor no abre las cuentas por cobrar, y la base no se toca", async () => {
    expect(EDITOR.has("finanzas.factura.ver")).toBe(false);
    sesion.permisos = EDITOR;
    await expect(CuentasPorCobrarPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(SinPermisoError);
    expect(getReceivablesKpis).not.toHaveBeenCalled();
    expect(listReceivables).not.toHaveBeenCalled();
  });

  it("el rol Editor tampoco abre el archivo de facturas", async () => {
    sesion.permisos = EDITOR;
    await expect(FacturasPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(SinPermisoError);
