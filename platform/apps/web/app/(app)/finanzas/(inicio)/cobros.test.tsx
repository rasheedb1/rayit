...
const estado: { rows: ReceivableRow[]; kpis: ReceivablesKpis; visto: unknown; recordatoriosPedidos: unknown } = {
  rows: [VENCIDA, VENCE_PRONTO, AL_DIA],
  kpis: KPIS,
  visto: null,
  recordatoriosPedidos: null,
};
...
vi.mock("@mc/db/queries/finanzas", () => ({
  ...
  // La bandeja de FIN-4 vive en la pantalla de cobro desde el cierre del
  // módulo; su contenido lo prueba bandeja.test.tsx, aquí solo se mira
  // que esté y con qué se pide.
  MAX_REMINDERS: 200,
  listReminders: async (_tx: unknown, params: unknown) => {
    estado.recordatoriosPedidos = params;
    return [];
  },
}));
...
describe("la pantalla de cobro reúne lo que hay que hacer hoy", () => {
  it("la bandeja de recordatorios (FIN-4) está en /finanzas y pide solo los pendientes", async () => {
    render(await pintar());
    expect(screen.getByRole("heading", { name: "Recordatorios por enviar" })).toBeInTheDocument();
    expect(screen.getByText("No hay recordatorios por enviar")).toBeInTheDocument();
    expect(estado.recordatoriosPedidos).toEqual({ pendingOnly: true, limit: 200 });
  });

  it("la tira del módulo tiene las seis vistas para quien lo ve todo, con Cobro activa", async () => {
    render(await pintar());
