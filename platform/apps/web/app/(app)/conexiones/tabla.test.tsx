...
import { TablaDeCuentas, type PermisosDeFila } from "./tabla";
...
/** El Dueño: puede todo. Las pruebas de permisos por rol van al final. */
const TODO: PermisosDeFila = { conectar: true, desconectar: true };

function pintar(rows: FilaDeCuenta[], entorno: EntornoDeConexion, permisos: PermisosDeFila = TODO) {
  render(<TablaDeCuentas rows={rows} ahora={AHORA} f={f} entorno={entorno} permisos={permisos} />);
}
...
describe("la tabla con oauth_connect encendida", () => {
  ...
  it("la fecha del snapshot es una columna `date`: no se corre un día en un workspace al oeste de Greenwich", () => {
    ...
    render(<TablaDeCuentas rows={[fila({ id: "6", latest })]} ahora={AHORA} f={bogota} entorno={CONFIGURADO} permisos={TODO} />);
...
/**
 * CON-B · lo que pinta la fila de cada costura con los módulos que
...
describe("las costuras en la fila", () => {
  ...
  it("ACC-8: «Conectada por X el Y» debajo del @, con la fecha del workspace", () => {
    pintar([fila({ id: "a1", handle: "delegada", connectedBy: { userId: "u2", name: "Andrés Pardo", email: null, at: "2026-09-20T15:00:00.000Z" } })], CONFIGURADO);
    expect(celdas("@delegada").getByText("Conectada por Andrés Pardo el 20 sep")).toBeInTheDocument();
  });

  it("CON-7: la fila dice qué falta, desde cuándo y por qué con el texto de la base, y enlaza el arreglo", () => {
    const porQue = "TikTok solo entrega la audiencia a la cuenta autorizada y con permiso de analítica. Para verla, el dueño tiene que autorizarla.";
    pintar([fila({ id: "g1", handle: "conhueco", gaps: [{ metricGroup: "demografia_de_cuenta", requirementId: "tt.audience.auth", messageEs: porQue, fixUrl: "https://support.tiktok.com/es/analytics", since: "2026-09-20" }] })], CONFIGURADO);
    const r = celdas("@conhueco");
    expect(r.getByText("Falta la audiencia de la cuenta")).toBeInTheDocument();
    expect(r.getByText(/desde el 20 sep/)).toBeInTheDocument();
    expect(r.getByText(porQue, { exact: false })).toBeInTheDocument();
    expect(r.getByRole("link", { name: MESSAGES.tabla.comoArreglarlo })).toHaveAttribute("href", "https://support.tiktok.com/es/analytics");
  });

  it("CON-5: «N en seguimiento» y hasta cuándo llegan las lecturas del contenido", () => {
    pintar([fila({ id: "p1", handle: "conposts", postsCount: 12, lastPostSnapshotAt: "2026-09-23T05:00:00.000Z", latest: { day: "2026-09-22", followers: 10, following: null, mediaCount: 140, views: null } })], CONFIGURADO);
    const r = celdas("@conposts");
    expect(r.getByText("12 en seguimiento")).toBeInTheDocument();
    expect(r.getByText(/publicaciones hasta el/).textContent).toContain("23 sep");
  });

  it("permisos: quien solo ve no tiene botones en la fila, y lo dice", () => {
    pintar([VENCIDA, POR_ARROBA], CONFIGURADO, { conectar: false, desconectar: false });
    expect(screen.queryByRole("button")).toBeNull();
    expect(screen.getAllByText(MESSAGES.tabla.sinAcciones)).toHaveLength(2);
  });
});
