...
export async function agregarCuenta(formData: FormData): Promise<void> {
  ...
  // TODO(ACC-2): audit('conexiones.cuenta.agregar', { after: { platformId, handle } }) — una cuenta conectada es escritura auditable.
...
export async function desconectarConexion(id: string): Promise<void> {
  ...
  // TODO(ACC-2): audit('conexiones.cuenta.quitar', { before: { platformId, handle } }) — sin tokens ni PII en before/after.
  if (!idSchema.safeParse(id).success) redirect("/conexiones");
  let error: string | null = null;
  try {
    await getCuentasService().quitar(id);
  } catch (err) {
    error = err instanceof ConnectionNotFound ? t.yaNoEsta : t.noSePudoQuitar;
  }
  revalidatePath("/conexiones");
