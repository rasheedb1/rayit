...
/**
 * Lo que ve quien entra a Resumen cuando la base falla en tiempo de
 * petición (DATABASE_URL inválida, statement_timeout, un
 * DEMO_WORKSPACE_ID que no es UUID…): un mensaje en español con un
 * botón para reintentar, no la página genérica de Next en inglés.
 *
...
export default function ResumenError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  useEffect(() => {
    ...
    console.error("[resumen] error al leer la base", error);
  }, [error]);
