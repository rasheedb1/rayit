...
export const DECISIONS: readonly { title: string; blocks: string[]; proposal: string; resolved?: string }[] = [
  ...
  },
  {
    ...
    resolved: "Resuelta el 22 de septiembre, con CIM-3: enlace mágico y todo el flujo en el servidor (@supabase/ssr), con la sesión en cookies httpOnly (lib/auth/cookies.ts). Quién eres sale siempre del correo VERIFICADO (migración sesion_correo_verificado) y de la cuenta de Auth que entró primero con él (app_user.auth_user_id); la cookie mc.workspace solo elige entre tus espacios. Sin sesión y con las llaves puestas no se sirve ningún espacio: lib/workspace/current.ts manda a /login aunque el middleware se lo salte. Hay cliente de navegador (lib/auth/supabase-browser.ts) pero ninguna pantalla lo usa: con la cookie httpOnly no ve la sesión, y es a propósito; si algún día hace falta la sesión en el navegador, se decide con él. Toca dos archivos de Nicolás, pendiente de su visto bueno en el PR de integración: components/shell.tsx monta <WorkspaceSwitcher /> (barra lateral y cabecera móvil) y su Brand pasa a ser <Marca enMarco /> de components/marca.tsx (una sola marca para el marco y la puerta), y app/layout.tsx dejó de montar el Shell, que bajó a app/(app)/layout.tsx para que /login no lleve marco; esto último es un cambio de estructura, no una línea de montaje. Si Nicolás no lo aprueba, la alternativa mínima es dejar el Shell en la raíz y que /login, /auth y /legal lo oculten con un segmento propio. Lo que hay que configurar en el panel de Supabase está en apps/web/README.md, «Autenticación».",
  },
  {
