...
export const DECISIONS: readonly { title: string; blocks: string[]; proposal: string; resolved?: string }[] = [
  ...
  },
  {
    ...
    resolved: "Resuelta el 22 de septiembre, con CIM-3: enlace mágico, todo el flujo en el servidor (@supabase/ssr) y middleware que protege (app)/. La sesión va en cookies httpOnly porque los dos clientes le pasan cookieOptions a @supabase/ssr (lib/auth/cookies.ts): por defecto la librería la escribe SIN HttpOnly y SIN Secure, y dentro van el access token y el refresh token; se puede porque no hay cliente de Supabase en el navegador, así que si algún día se añade uno hay que revisarlo con él. Quién eres sale siempre del correo verificado (app.user_email + política de 0022); la cookie mc.workspace es solo la preferencia de espacio y únicamente se respeta si ese espacio está entre los que la base devuelve para tu correo. Toca dos archivos de Nicolás: `components/shell.tsx` monta `<WorkspaceSwitcher />` en la barra lateral y en la cabecera móvil (dos líneas, porque el marco tiene dos cabeceras), y `app/layout.tsx` dejó de montar el Shell, que bajó a `app/(app)/layout.tsx` para que /login no lleve marco. Falta que una persona ponga el Site URL y las Redirect URLs en el panel de Supabase (apps/web/README.md, «Autenticación»), y un SMTP propio cuando el correo integrado se quede corto.",
  },
  {
