...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "hecho",
    note: "Hecha el 23-sep sobre la pantalla de CON-10. El estado NO se copia de social_connection.status: se compara access_expires_at con el reloj, porque oauth.refresh todavía no corre en producción (CIM-7) y una cuenta con el token ya vencido seguía viéndose verde. «Conectar», «Reautorizar» y el paso manual viven detrás de oauth_connect (OAUTH_CONNECT=1); con la bandera apagada —lo que hay hoy en producción— la pantalla es la de CON-10 con el estado, la frescura y la columna «Acceso» nuevos. Probada con el escenario del --demo del worker contra Postgres embebido (pagina.test.tsx) y 17 pruebas de las funciones puras. Ya no hay condición pendiente: CON-3 se probó en vivo el 23-sep y OAUTH_CONNECT=1 está en producción con las credenciales de TikTok, así que al mergear se ve. «Conectar» solo ofrece TikTok: es la única red donde autorizar añade algo (Instagram ya entrega seguidores y publicaciones por @). Rebasada sobre el main con ACC-1: la pantalla abre con requirePermission('conexiones.cuenta.ver') y el POST de start con 'conexiones.cuenta.conectar'; la bitácora queda en TODO(ACC-2). Detalle en docs/propuestas/CON-4.md.",
  },
  {
