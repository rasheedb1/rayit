...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "hecho",
    note:
      "ESTADO (22-sep, pulido r1): guardia de esquema en verde sobre 0001–0030 en embebido; ahora distingue el padre que aísla por persona (app_user, membership, workspace), mira las funciones definer de extensions y exige el UPDATE de workspace solo por columna (el plan ya no lo cambia el propio workspace). PENDIENTE DEL INTEGRADOR, en este orden: crear mc_public_share con supabase-admin.sh; make db.migrate (0024, 0025, 0026, 0027, 0028, 0029, 0030; Supabase va por 0022); make db.guardia en verde ANTES de make vercel.deploy PROD=1 (en producción la web no arranca si la guardia reporta algo), y pegar aquí su salida. ABIERTO, con plan en docs/propuestas/CIM-2.md §3: ids bigserial como contador global (pasar a uuid), UPDATE de account_metric_snapshot por el upsert de CON-10 (a DO NOTHING o al worker, luego REVOKE) y el radar, que no puede publicar sin dueño lo que salga de una lista privada. Historia por rondas: docs/propuestas/CIM-2.md §6.1.",
  },
  {
    ...
    status: "hecho",
    note:
      "ESTADO (22-sep): integrada con el endurecimiento. 0028 empieza con una guardia que para si 0024 no está en schema_migrations y engancha assert_reference_visible en membership (mc_app vuelve a tener INSERT); getWorkspace filtra por current_workspace_id() porque workspace_read_member deja ver los otros espacios de la persona. Pendientes de aplicar en Supabase (va por 0022_public_profile_access), con make db.migrate y en este orden: 0024_aislamiento_por_defecto, 0025_referencias_visibles, 0026_duenos_unicos_secuencias, 0027_sesion_correo_verificado, 0028_membership_alta_propia, 0029_supresion_verificada_filas_que_nombran y 0030_public_share (antes, el rol mc_public_share; ver su cabecera). make db.check las aplica todas en orden. Hecha el 22 de septiembre (rondas 3 y 4). Enlace mágico con Supabase Auth, todo en el servidor; quién eres sale del correo verificado y, desde la ronda 4, de la cuenta de Auth: app_user.auth_user_id liga la fila a la cuenta que entró primero y otra cuenta con el mismo correo (un buzón reasignado) no la hereda. La cookie mc.workspace solo elige entre tus espacios y sin sesión no se sirve ningún espacio. Ronda 4: una sola llamada a Supabase Auth por petición (el middleware verifica y deja la sesión en una cabecera interna que borra si la trae el navegador); con Supabase caído ya no se manda a /login, se cae en error.tsx; /login con sesión es un 307 desde el middleware; el enlace con token_hash se canjea con un clic en /auth/confirm (POST), porque los escáneres del correo corporativo lo gastaban; error_code=otp_expired dice «ese enlace ya no sirve»; «Reenviar» espera 60 s a la vista y un fallo ya no saca de «Revisa tu correo»; el selector es accesible con teclado (menuitemradio con aria-checked, el actual enfocable, «Crear espacio» fuera del menú). MIGRACIONES: 0027_sesion_correo_verificado y 0028_membership_alta_propia, sin aplicar en Supabase; van detrás de 0024–0026 (0024 §7 hace REVOKE INSERT ON membership y 0028 lo devuelve, así que el orden importa). El código no cita su número. Probado en PGlite con 0022 de main y 0024/0025 de endurecer-db: packages/db/test/identidad.test.ts pasa entero; el alta de app_user ya fija id = current_user_id(), que es lo que exige la política de 0025 §4. A mano, una vez, en el panel de Supabase: Redirect URLs con /**, plantilla del enlace hacia /auth/confirm y Confirm email (apps/web/README.md#autenticación); SUPPORT_EMAIL en Vercel. Pendiente de Nicolás: usar components/marca.tsx en la Brand de shell.tsx (una línea; no se tocó sin su visto bueno). Fuera: invitar a otras personas, cambiar de correo, Google, y los términos legales (CIM-9). Integrada en rasheed/integracion (fase 2, 22-sep) y después con el endurecimiento: la 0026_public_share de Cotizar pasó a 0030_public_share, y todo lo pendiente se aplica en el orden de la nota de CIM-2.",
  },
  {
  ...
  // ---------------------------------------------------------------- RES
  {
    ...
    status: "hecho",
    note: "Pantalla real sobre datos reales, con toda la aritmética en SQL (queries/resumen.ts): KPIs con delta y sparkline, dos gráficos del kit con su tabla, frescura por conexión y estado vacío en vez de ceros. Decisiones que importan: las sumas de la cuenta terminan en su último día cerrado, la comparación es entre las mismas cuentas y los días son días cerrados en UTC (detalle en docs/fases-rasheed.md §8). Desviación del criterio: barras por bloques de 1/5/10 días en vez de 12 semanas; pendiente de que BarChart acepte elegir etiquetas (Nicolás), en RES-5.",
  },
  {
    ...
    status: "hecho",
    note: "Asistente de 4 pasos en /resumen/importar que guarda la lectura con la fecha de la exportación y nunca hacia atrás. Decisión que importa: el mapeo manual es el camino, porque ninguna plataforma documenta la cabecera de su exportación (detalle en docs/fases-rasheed.md §8).",
  },
  {
    ...
    status: "bloqueada",
    note: "Espera a Nicolás: que BarChart deje elegir qué etiquetas pinta (axisLabels ya permite etiquetas propias, no decidir cuáles se dibujan) y decidir si el periodo largo pasa a 84 días o el gráfico deja de cuadrar con la tarjeta.",
  },
  {
  ...
  },
  {
    ...
    status: "hecho",
    note: "/kit/<slug> sin sesión, sobre public_media_kit() (migración 0030, rol mc_public_share; la web la abre con db.withPublicShare, sin forzar tipos). Contraseña de 8 signos como mínimo, guardada como huella scrypt con sal por fila, 5 intentos por minuto por IP y bloqueo de 15 minutos por enlace tras 10 fallos en la base (compromiso aceptado, explicado en la cabecera de 0030). Vence al final del día en la zona del workspace. La vista previa del panel y los robots de WhatsApp o Slack no cuentan visitas. Audiencia por red y dimensión, con «Otros» siempre al final. La cifra grande de views es la de la mejor red y se rotula con ella; los totales salen de SQL. El documento declara el idioma de sus textos (MVP en español, decisión en el README). Prueba: subir los seguidores después de generarlo no cambia lo que ve la marca. Integrada en rasheed/integracion (fase 2, 22-sep) y después con el endurecimiento: la 0026_public_share de Cotizar pasó a 0030_public_share, y todo lo pendiente se aplica en el orden de la nota de CIM-2.",
  },
  {
