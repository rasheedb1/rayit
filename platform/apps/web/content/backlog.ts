...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "en_curso",
    note: "Rama nicolas/FIN-6-flujo-caja, lista para PR. projectCashflow() en packages/core/src/flujo-caja.ts (34 pruebas), getCashflowInputs() —una sola consulta— en queries/finanzas.ts (25 pruebas en PGlite con el seed, con la negativa de RLS) y la pantalla /finanzas/flujo (11 pruebas). FIN-2 y FIN-5 no hacían falta: lo que FIN-6 necesita de ellas son las tablas payment/expense (0008) y el seed 0003, no sus pantallas; flujo-caja.ts lo crea esta historia. Sin migración. Queda ABIERTO el criterio del rol Mánager: requirePermission no existe todavía en ninguna rama (es ACC-5) y ACC-1 no está en main, así que la pantalla lleva su TODO(ACC-1): finanzas.flujo.ver y lo probado hoy es el aislamiento por workspace. Decisión pendiente de Nicolás en docs/propuestas/FIN-6.md §0.2.4: el cobro esperado es el bruto de la factura, no el neto de retención.",
  },
  {
