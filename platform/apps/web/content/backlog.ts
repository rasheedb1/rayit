...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "pendiente",
    note: "membership YA tiene RLS (migración 0019, ronda 4 de CIM-2): ENABLE + FORCE con USING (workspace_id = current_workspace_id() OR user_id = current_user_id()), y current_user_id() existe leyendo app.user_id —devuelve NULL hasta que alguien lo fije—. Lo que falta aquí: (1) fijar app.user_id por transacción, como withWorkspace fija app.workspace_id, con lo que «a qué workspaces pertenezco» se responde como mc_app sin asWorker ni SECURITY DEFINER; (2) app_user YA tiene RLS desde la migración 0020 (ronda 5 de CIM-2): SELECT con «soy yo (current_user_id()) o comparto workspace conmigo» e INSERT aparte, que es lo que resuelve el orden del seed sin esperar a esta historia; aquí solo queda que al fijar app.user_id la rama «soy yo» empiece a valer, y abrir el UPDATE de «edito mi perfil» con id = current_user_id(); (3) reemplazar el cuerpo de apps/web/lib/workspace/current.ts (DEMO_WORKSPACE_ID) por el workspace de la sesión: es el único sitio.",
  },
  {
  ...
  // ---------------------------------------------------------------- RES
  {
    ...
    status: "hecho",
    note: "Pantalla real sobre datos reales en app/(app)/resumen/, con todas las consultas en packages/db/src/queries/resumen.ts y toda la aritmética en SQL: ninguna cifra se calcula en React. Con el seed da las cuatro del mock (412 mil seguidores, 2,6 M de visualizaciones en 30 días, 56 % de alcance en no seguidores, 13,3 guardados por mil), cada una con su variación contra el periodo anterior y una sparkline de doce ventanas deslizantes cuyo primer punto ES el periodo anterior, así que la línea y el delta nunca se contradicen. El periodo (7/30/90) y la red viven en la URL, así que el enlace es compartible. Dos gráficos del kit (LineChart y BarChart en SVG, sin librería nueva) con su «Ver tabla», tooltip con el número exacto y «datos hasta el {fecha}»; una tarjeta de frescura por conexión con la fuente de la última lectura y el aviso de permiso por vencer; error.tsx, loading.tsx y Suspense por bloque. Dos decisiones que importan: el reloj del módulo es el último DÍA CERRADO (max(day) de account_metric_snapshot) y no now(), para que el aviso de frescura no diga «hoy» con un día a medias; y una ventana que la historia no cubre sale NULL y no cero —con periodo de 90 y noventa días de seed, la comparación de las sumas no existe y se dice, en vez de inventar una subida. El plan de construcción del módulo se mudó a /plan/[modulo] y se enlaza desde la cabecera. Textos en resumen/messages.ts; fechas y cifras con el locale y la zona del workspace. Ronda 2: el reloj del módulo mira las DOS fuentes —serie de cuenta y lecturas de contenido— para que un workspace que solo importó un CSV vea cifras y no una pantalla en blanco, y los dos KPIs de contenido, que son razones, ya no cuelgan del primer día de la serie de cuenta; la ventana de los gráficos la decide EL PERIODO y su único suelo es el primer día con datos del workspace, así que conectar hoy una cuenta nueva ya no recorta la serie de las que llevan meses midiendo; el eje de seguidores arranca siempre en cero, porque arrancar en el mínimo dejaba marcas como «216,1 mil» que no caben en el margen del eje del kit; las barras pasan a 9 de diez días en 90 días y 7 de cuatro en 30, porque BarChart etiqueta cada ceil(n/8) categorías y ADEMÁS la última, y con doce barras esas dos etiquetas se pisaban (conservar las doce semanas del mock pedía cambiar la API del kit); las cuatro tarjetas de KPI explican igual la ausencia de comparación, en vez de que dos la callen; la tarjeta de frescura tiene su propio Suspense, que es lo que hace verdaderos los dos esqueletos de arriba, y muestra la fecha de la última lectura de contenido cuando no hay serie de cuenta; el estado vacío de cada gráfico ofrece la salida que de verdad existe y no «Ver 90 días» cuando ya estás en 90; y la cobertura dejó de contar la tabla de posts entera en el camino crítico de la primera consulta. Ronda 3: un workspace que solo importó CSV ve los cuatro KPIs con sentido —alcance y guardados sobre sus videos, visualizaciones de lo publicado en el periodo con la nota que lo dice, seguidores con «llegan al conectar la cuenta»— y el gráfico de barras por fecha de publicación; el de seguidores ofrece conectar la cuenta en vez de «otro periodo». Una lectura tomada hoy describe hasta ayer, así que importar a media tarde ya no mueve el reloj ni resta un día a las visualizaciones. Cada razón se calcula solo sobre los videos que traen sus dos términos (un CSV sin alcance en no seguidores diluía el KPI) y dice sobre cuántos, en su vida completa en los dos periodos: post_metrics_at_cut no trae alcance en no seguidores y un video importado tiene una sola lectura, así que un corte de edad dejaba fuera todo lo importado. La frescura respeta el filtro por red y enseña por separado la fuente de la conexión y la del CSV. Etiquetas del eje en número («16/9») para que quepan a 400 px, y la nota de redes desalineadas solo con varias redes. Los identificadores de @mc/db pasan al inglés (getFollowersByPlatform, importCsvReadings…) y las redes y periodos viven en queries/resumen-constantes, sin dependencias, para no arrastrar el cliente de Postgres al navegador.",
  },
  {
    ...
    status: "hecho",
    note: "Asistente de cuatro pasos en /resumen/importar (subir → formato → revisar → importar), al estilo de Flatfile. El archivo se lee y se valida EN EL NAVEGADOR y solo viaja al servidor al confirmar; la server action vuelve a leerlo y a validarlo con las mismas funciones puras, porque un POST a mano no pasa por la pantalla. Escribe post y post_metric_snapshot con source = csv_import y age_hours calculada por Postgres (captured_at − published_at), nunca por el reloj del navegador; las lecturas son append-only, así que subir el mismo archivo dos veces añade lecturas y no duplica el video. La cuenta de destino puede ser una conexión existente o una nueva con access_mode = manual_csv (previsto en 0002), sin ningún token. HALLAZGO: ninguna de las tres plataformas publica el encabezado exacto de su exportación y las tres lo cambian según informe, idioma y columnas visibles, así que el detector solo adivina la RED por firma y el mapeo va por una tabla de alias en español e inglés; lo que no reconoce lo asigna la persona, y ese es el camino previsto, no la excepción. Cinco fixtures en apps/web/test/fixtures/csv/ (las tres exportaciones, una cabecera desconocida en español con punto y coma, y una con filas sucias) y 28 pruebas del parser: miles con punto o con coma, fechas ISO y d/m/a en la zona del workspace, id sacado del enlace, impresiones que NO son alcance, duplicadas dentro del archivo, fecha futura y alcance en no seguidores mayor que el alcance. Ronda 2: el paso 3 ya marca los DUPLICADOS CONTRA LA BASE —una acción de solo lectura pregunta por los ids del archivo en la cuenta de destino y el encabezado dice «N ya estaban»—, así que nadie confirma una escritura sin saber qué va a pasar; una fecha numérica se lee con el orden del locale del workspace (m/d/a solo donde toca) y, cuando los dos órdenes son posibles, la fila lleva un aviso que dice cómo se leyó; las filas que no entran enseñan su celda cruda para poder buscarlas en Excel; /resumen/importar tiene su loading.tsx y su error.tsx propios, porque heredaba el esqueleto y el mensaje de error de Resumen, que aquí serían mentira; y un archivo de entre 1 y 5 MB ya no muere en silencio: serverActions.bodySizeLimit sube a 6 MB, el techo del servidor se mide con Buffer.byteLength y la llamada va en try/catch, para que ningún fallo de transporte se lleve por delante el archivo, el mapeo y la revisión. Ronda 3: el orden día/mes se decide para el ARCHIVO entero (una fecha con un número mayor que 12 lo demuestra) y, si ninguna lo demuestra, el paso 2 pregunta con un Segmented, propone el del workspace y enseña una fecha del archivo leída; también entiende «8:15 PM», que antes se leía como las 8:15 de la mañana. Fixtures nuevos: TikTok con cuenta en-US (mes/día) y uno ambiguo. importCsvReadings se protege sola —isUuid, rechaza un lote con el mismo video dos veces, cuenta los nuevos con INSERT … RETURNING, captured_at al microsegundo para que dos importaciones no empaten— y solo marca last_synced_at en cuentas manual_csv: un CSV sobre una conexión OAuth ya no tapa un recolector caído. Una cuenta importada que se borró no resucita con su historial, y nunca se cuelga de un creador borrado. Los rechazos viajan como código (CsvImportError, ErrorCsv, códigos de fila) y la pantalla los traduce: todos los textos, incluidos los de validación, los nombres de campo y de formato, están en messages.ts, y los contadores salen con formatInt. La server action tiene su propia prueba contra Postgres embebido (entrada inválida, mapeo sin fecha, cuenta de otro workspace, archivo por encima del techo) y otra fija que bodySizeLimit cubre MAX_BYTES.",
  },
  {
