...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "hecho",
    note: "projectCashflow() en packages/core/src/flujo-caja.ts (34 pruebas), getCashflowInputs() —una sola consulta— en queries/finanzas.ts (25 en PGlite con el seed, con la negativa de RLS) y /finanzas/flujo (13). La pantalla abre con requireModuleAccess y requirePagePermission(\'finanzas.flujo.ver\'): el rol Mánager recibe 404 (ACC-5, ya en main) y una prueba comprueba que ni siquiera se lee la base. FIN-2 y FIN-5 no hacían falta: lo que FIN-6 necesita de ellas son las tablas payment/expense (0008) y el seed 0003, no sus pantallas; flujo-caja.ts lo crea esta historia. Sin migración. Falta solo la decisión de docs/propuestas/FIN-6.md §0.2.4: el cobro esperado es el bruto de la factura, no el neto de retención.",
  },
  {
    ...
    status: "hecho",
    note: "En /finanzas/ingresos: importar CSV (AdSense mensual o diario, Creator Rewards y una lista genérica), agregar a mano, lista por mes y el estimado mensual; y en /finanzas/flujo, la columna «Otros ingresos» con su barra y su nota. La idempotencia es de la base: el UNIQUE natural de la migración 0036, aplicada en Supabase; en producción desde el 23-sep (7737b62). Las tres páginas abren con la puerta de ACC-5 (404 sin permiso) desde P0. El promedio vive en packages/core/src/ingresos-plataformas.ts y entra a projectCashflow (FIN-6) por otrosIngresosMensual, que es opcional: sin estimado, el flujo se comporta igual que antes. Se reutilizan finanzas.flujo.ver y finanzas.pago.registrar en vez de crear finanzas.ingreso.*: el catálogo viaja en la semilla de 0034 y accesos.test.ts exige que sea idéntica (docs/propuestas/FIN-7.md §1.2). Propuesta: docs/propuestas/FIN-7.md.",
  },
  {
