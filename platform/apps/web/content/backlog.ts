...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "bloqueada",
    note: "Construida y probada el 23-sep contra respuestas grabadas (worker 10, db 4, la lógica de prerrequisitos en milisegundos). Falta SOLO la prueba en vivo: ninguna fuente pública da demografía —business_discovery no la trae, YouTube la da por Analytics con OAuth y TikTok por la Accounts API con el trámite CON-9—, y OAUTH_CONNECT sigue apagado desde la decisión del 22-sep, así que hoy no hay ninguna conexión autorizada con los permisos de insights. El job escribe audience_breakdown con scope 'account' y, cuando falta un prerrequisito, NO llama a la API: escribe el requisito en metric_gap (migración 0036) con el message_es de metric_requirement, que es lo que lee RES-4 por getAccountAudience. 0036 añade además el UNIQUE que le faltaba a audience_breakdown y cinco filas de metric_requirement, entre ellas el requisito nuevo owner_authorization para las cuentas agregadas por @. Detalle en docs/propuestas/CON-7.md.",
  },
  {
