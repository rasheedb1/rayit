...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "hecho",
    note:
      "Hecha el 22 de septiembre (rondas 3 y 4). Enlace mágico con Supabase Auth, todo en el servidor; quién eres sale del correo verificado y, desde la ronda 4, de la cuenta de Auth: app_user.auth_user_id liga la fila a la cuenta que entró primero y otra cuenta con el mismo correo (un buzón reasignado) no la hereda. La cookie mc.workspace solo elige entre tus espacios y sin sesión no se sirve ningún espacio. Ronda 4: una sola llamada a Supabase Auth por petición (el middleware verifica y deja la sesión en una cabecera interna que borra si la trae el navegador); con Supabase caído ya no se manda a /login, se cae en error.tsx; /login con sesión es un 307 desde el middleware; el enlace con token_hash se canjea con un clic en /auth/confirm (POST), porque los escáneres del correo corporativo lo gastaban; error_code=otp_expired dice «ese enlace ya no sirve»; «Reenviar» espera 60 s a la vista y un fallo ya no saca de «Revisa tu correo»; el selector es accesible con teclado (menuitemradio con aria-checked, el actual enfocable, «Crear espacio» fuera del menú). MIGRACIONES para el integrador, SIN aplicar en Supabase: 0027_sesion_correo_verificado y 0028_membership_alta_propia. Van DETRÁS de todo: 0022 ya está en Supabase (main, public_profile_access), 0023 es de ACC-3, 0024 y 0025 de endurecer-db (0024 §7 hace REVOKE INSERT ON membership y 0028 lo devuelve, así que el orden importa), 0026 queda para COT-1 (hoy 0022_public_share en su rama). Si algún número de delante cambia, estas se renumeran con él ANTES de make db.migrate; el código no cita su número. Probado en PGlite con 0022 de main y 0024/0025 de endurecer-db: packages/db/test/identidad.test.ts pasa entero; el alta de app_user ya fija id = current_user_id(), que es lo que exige la política de 0025 §4. A mano, una vez, en el panel de Supabase: Redirect URLs con /**, plantilla del enlace hacia /auth/confirm y Confirm email (apps/web/README.md#autenticación); SUPPORT_EMAIL en Vercel. Pendiente de Nicolás: usar components/marca.tsx en la Brand de shell.tsx (una línea; no se tocó sin su visto bueno). Fuera: invitar a otras personas, cambiar de correo, Google, y los términos legales (CIM-9). Integrada en rasheed/integracion (fase 2, 22-sep): 0026, 0027 y 0028 pasan make db.check pero NO se aplicaron en Supabase, porque 0024 y 0025 del endurecimiento van delante y todavía no están integradas; se aplican juntas, en orden, al integrar el endurecimiento.",
  },
  {
