...
export const STORIES: readonly Story[] = [
  ...
  },
  {
    ...
    status: "pendiente",
    note: "membership YA tiene RLS (migración 0019, ronda 4 de CIM-2): ENABLE + FORCE con USING (workspace_id = current_workspace_id() OR user_id = current_user_id()), y current_user_id() existe leyendo app.user_id —devuelve NULL hasta que alguien lo fije—. Lo que falta aquí: (1) fijar app.user_id por transacción, como withWorkspace fija app.workspace_id, con lo que «a qué workspaces pertenezco» se responde como mc_app sin asWorker ni SECURITY DEFINER; (2) la RLS de app_user, que no pudo entrar en 0019 porque el seed crea app_user antes que su membership y con FORCE ese INSERT fallaría (la política redactada está en el test.todo de packages/db/test/schema.test.ts); (3) reemplazar el cuerpo de apps/web/lib/workspace/current.ts (DEMO_WORKSPACE_ID) por el workspace de la sesión: es el único sitio.",
  },
  {
  ...
  // ---------------------------------------------------------------- VEN
  {
    ...
    status: "pendiente",
    note: "La RLS de contact ya NO es parte de esta historia: la adelantó la migración 0019 en la ronda 4 de CIM-2 (USING con source pública o company_link del workspace), con su caso en packages/db/test/rls.test.ts. Aquí queda la pantalla; los contactos se leen y se escriben dentro de withWorkspace y la política hace el resto.",
  },
  {
