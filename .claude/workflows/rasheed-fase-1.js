...
// Fase 4 · tubería        esquema (0015) → canales (VEN-9) · motor (VEN-10) · entregabilidad (VEN-15)
...
export const meta = {
  ...
  phases: [
    ...
    { title: 'Integración 4', detail: 'merge, migración 0015, CI' },
...
`,
  ...
  esquema: {
    ...
    brief: `
QUÉ CONSTRUYES: la migración platform/db/migrations/0015_outreach.sql y su esquema Drizzle, exactamente como la describe docs/ventas-outreach.md sección 5.2. Nada más: es una pieza corta que las demás de esta fase necesitan integrada antes de empezar.
...
- Verifica con \`make db.check\`; NO la apliques a Supabase. Actualiza packages/db/src/schema con las tablas y funciones nuevas y agrega una prueba en pglite que aplique 0015 y ejercite increment_if_under_cap con dos llamadas concurrentes simuladas y public_optout.
TERMINADO CUANDO: \`make db.check\` aplica 0015 en limpio; la prueba de caps y de baja pasa; typecheck en verde.
`,
  },
  canales: {
...
- Interruptor: outbound_policy.enabled apagado cancela lo pendiente (usa disable_outreach de 0015) y el despachador no toma nada.
...
- Página pública apps/web/app/(public)/baja/[token]/page.tsx: sin sesión, un solo botón «Dejar de recibir mensajes», confirma y llama a public_optout(token) de 0015; muestra un estado claro después. Referencia: la página de baja de Substack (una frase, un botón, sin trucos).
- Job apps/worker/src/jobs/ventas/outbound.bounces.ts: lee rebotes con el conector de Gmail (a través de su interfaz; no toques packages/connectors), marca contact.email_invalid con motivo (agrega la columna en 0016_contact_email_invalid.sql si no existe; verifícala con make db.check) y cancela los toques de correo pendientes de ese contacto.
- Job apps/worker/src/jobs/ventas/outbound.alerts.ts: a diario, con outbound_health de 0015: rebotes sobre el 5 % con diez intentos o más, cero envíos con enrolamientos activos, cola atascada, cuenta de canal caída, presupuesto de LLM agotado; una notification por tipo y día y un correo de resumen al dueño del workspace por SMTP_URL (Mailpit en local). Prueba con fixtures.
...
- Plantillas globales en outbound_sequence_template (de 0015) por nicho y tipo de señal: al menos seis, escritas con criterio, en una migración 0017_secuencias_plantilla.sql verificada con make db.check.
