...
- El producto se llama **On Cue** (antes MultiCampaign; el nombre viejo sobrevive en algún documento, no lo propagues).
- Monorepo pnpm en platform/ (Next.js 15 en apps/web, packages/db con Drizzle, packages/core, packages/connectors, apps/worker con pg-boss sobre Postgres; base en Supabase con migraciones ya aplicadas e inmutables). Lee CLAUDE.md, docs/backlog-mvp.md, docs/fases-rasheed.md y, para todo lo de Ventas, docs/ventas-outreach.md antes de tocar nada.
- **Ya existe y NO lo construyes tú:** el marco de la aplicación y la navegación (CIM-4), el kit de interfaz en apps/web/components/ui/ (CIM-5, con su README y su galería en /kit), el cliente de base @mc/db con withWorkspace y aislamiento por fila (CIM-2), los seeds, el worker con su runner de pg-boss (CON-2) y los conectores grabados (CON-1). **Usa el kit: Button, Pill, PlatformPill, Field/Input/Select/Textarea, MoneyInput, DateInput, Segmented, EmptyState, DataAsOf, Kpi/KpiRow, DataTable/CellMain, LineChart, BarChart, ChartCard.** Lee components/ui/README.md antes de escribir una pantalla. Agregar un componente al kit es libre; cambiar la API de uno existente pide PR revisado por Nicolás, así que evítalo.
- **La pantalla de referencia de un módulo real es Finanzas** (apps/web/app/(app)/finanzas/): mírala para el patrón de lista, formulario, detalle, error.tsx, loading.tsx y messages.ts. Copia ese patrón, no inventes otro.
- Formato de cifras y fechas: apps/web/lib/format.ts (formatMoney, formatInt, formatCompact, formatPct, formatDelta, formatDate) con Intl y el locale, la moneda y la zona del workspace. Nunca formatees a mano.
- La puerta de calidad del repositorio es \`pnpm verificar\` en platform/ (typecheck, lint y test con concurrencia acotada) más \`pnpm --filter @mc/web build\`.
...
1. Funciona (peso 3). \`pnpm verificar\` y \`pnpm --filter @mc/web build\` en verde en la rama. Si algo falla, la nota máxima es 5.
...
- Cambio de workspace: components/workspace-switcher.tsx (nombre, inicial, lista de workspaces, «Crear espacio»), montado en components/shell.tsx con una sola línea (es carpeta de Nicolás: lee primero cómo quedó shell.tsx, monta con el mínimo cambio posible y déjalo dicho en decisions). Referencia: el selector de Notion y el de Vercel arriba a la izquierda.
...
- Interfaz, **toda con el kit que ya existe** (KpiRow, Segmented, ChartCard + LineChart/BarChart, DataAsOf, EmptyState, DataTable): selector de periodo 7 / 30 / 90 días en la URL (compartible), filtro por red, fila de KPIs con delta y sparkline, dos gráficos con tooltip exacto y el botón «Ver tabla» que ChartCard ya trae, aviso de frescura por conexión, estado vacío con dos acciones («Conectar una cuenta», «Importar un CSV»), esqueletos de carga con Suspense, y error.tsx y loading.tsx del módulo como los de Finanzas. No agregues una librería de gráficos: el kit tiene los suyos en SVG con los tokens del tema.
...
- COT-1 pantalla, con el kit que ya existe (DataTable, MoneyInput, Field, Button, Pill): tabla de entregables con rangos (no precios fijos), panel «Cómo se calcula» al estilo del desglose de comisiones de Stripe, edición en línea con marca de «editado». Las views salen de creator_baseline cuando es confiable; si no, el creador las escribe y quedan marcadas como manuales (D4). Con error.tsx, loading.tsx y messages.ts como los de Finanzas.
