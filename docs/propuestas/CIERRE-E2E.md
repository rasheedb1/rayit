...
| Supabase | Al empezar, `0001`–`0039` (`0023` hueco), con la `0041` (CAM) en `main` sin aplicar y la `0040` (ACC-6) en la rama de ACC. **A las 00:37 UTC del 24 se aplicaron la 0040 y la 0041.** La **0042** (CON-C) entró después y también está aplicada |
...
declarar». CON-C aplicó después la **0042** y dejó la guardia en verde
(`CIERRE-CON-C.md` §9.1). Desde el clon principal da rojos falsos (compara con código
de `29460e3`).
...
| 5 | **OAuth de YouTube** (CON-8) y **proveedor de TikTok** (CON-12) | En `main` y apagados: sin sus variables, la pantalla no los ofrece y lo dice | CON-8: `GOOGLE_CLIENT_ID/SECRET` en Vercel, GitHub y el vault, y autorizar **un canal de prueba** con la app en «Testing» (basta para su «terminado»). La verificación de Google (CON-9) solo hace falta para creadores reales. CON-12: decidir si se contrata EnsembleData | **Nicolás** (CON-8, CON-12); **Rasheed** (CON-9, solo para creadores reales) |
| 6 | **Demografía en vivo** (CON-7) | La pantalla dice qué requisito falta | **No depende de CON-9** (corregido el 24-sep). La mitad TikTok del «terminado» sale con la primera corrida de `collect.demographics` (fila 1): @selvathegolden deja su hueco con la frase. La tabla real sale de un canal de YouTube **de prueba** autorizado con CON-8 (fila 5), al día siguiente de autorizar (`CIERRE-CON-C.md` §3) | **Nicolás** (tras las filas 1 y 5) |
...
24-sep, tras los merges de los cierres de CON-C y ACC y la 0042: **15/15
