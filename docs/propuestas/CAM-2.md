...
3. **Reglas.** La cotización debe estar en `'accepted'`: la llamada va
   ...
   `0016_campaign_quote_unique.sql`** (nació como 0015; renumerada al integrar porque `0015_connection_secret.sql` de CON-3 ya estaba aplicada), índice único parcial sobre
...
- **Idempotencia.** Con el mismo `quoteId` devuelve la campaña existente
  ...
  único parcial de 0016 no deja dos campañas activas por cotización.
...
- [ ] Aplicar `db/migrations/0016_campaign_quote_unique.sql` en
...
- `pnpm --filter @mc/db test`: 36 (9 de CAM-2): crea con los campos
  ...
  campaña cancelada libera la cotización y el índice de 0016 rechaza un
...
|---|---|
| La idempotencia dependía solo del bloqueo y de un SELECT | Migración 0016: índice único parcial sobre `campaign (quote_id)` salvo canceladas |
