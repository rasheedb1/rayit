...
>
> **Nota del cierre del módulo (23-sep, tarde).** La rama se integró en
> `nicolas/ACC-cierre-modulo`. La tabla `membership_scope` la creó
> `0034_access_control.sql` de ACC-3 (§6 y §10, mismas columnas y
> política), así que la migración de esta historia quedó en
> **`0040_scope_allows.sql`**, con solo lo que 0034 no trae: el índice
> `campaign_post (post_id)` y las dos firmas de `scope_allows()`. D1 queda
> resuelta por los hechos. El alcance cubre ahora TODAS las funciones de
> Campañas, Finanzas y Conexiones (no solo las 34 de aquí), con una prueba
> de convención estática. Todo eso, las decisiones y lo que falta de
> Rasheed: `docs/propuestas/CIERRE-ACC.md`. Lo de abajo es la historia tal
> como se escribió; donde dice 0035, léase 0040.
...
## 2. La migración `0035_membership_scope.sql` (hoy `0040_scope_allows.sql`, solo la sección 3)
...
1. ~~**0034 aparte o dentro de ACC-3** (D1).~~ Resuelta en el cierre: ACC-3 absorbió la tabla y aquí quedó 0040 con las funciones.
