...
| CON-4 | Pantalla Conexiones sobre `connection_health`: conectar, estado, horas desde la última sincronización, paso manual de Analytics en TikTok. | M | CON-3, CIM-5 | Una conexión vencida se ve en rojo con el botón de reautorizar. **Hecha el 23-sep**, sobre la pantalla de CON-10; «Conectar», «Reautorizar» y el paso manual detrás de `oauth_connect`. Encenderla pide la prueba en vivo de CON-3 §5. |
